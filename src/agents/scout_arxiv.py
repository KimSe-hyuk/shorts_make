"""ArXiv 논문 수집을 담당하는 The Scout 에이전트."""
...
DEFAULT_QUERIES = [
    ["History of Science"],
    ["historical persona", "decision pattern"],
...
async def collect_arxiv_papers(
    ...
    async with httpx.AsyncClient(timeout=DEFAULT_TIMEOUT) as client:
        ...
        async def fetch_query(keywords: list[str]) -> Tuple[bool, str]:
            ...
            # arXiv 레이트 리밋(3초/단일 연결)을 보장하기 위한 락 사용.
            return await fetch_arxiv_feed(client, query_url, lock, last_request)
    ...
    for keywords, (ok, payload) in zip(queries, results):
        if not ok:
            errors.append(f"{keywords}: {payload}")
            continue
        try:
            papers = _parse_arxiv_feed(payload)
    ...
    if errors:
        await log_event(f"ArXiv 수집 실패: {' | '.join(errors)}")
        return False, "; ".join(errors)

    await log_event(f"ArXiv 수집 성공: {len(papers_payload)}개 쿼리")
    return True, _safe_json_dumps(papers_payload)


async def run_scout() -> Tuple[bool, str]:
    """ArXiv 논문 정보를 수집하고 보고서를 생성한다.

    Returns:
        성공 여부와 보고서 텍스트.
    """
    ok, payload = await collect_arxiv_papers(
        DEFAULT_QUERIES,
        max_results=5,
    ...
    output_lines = ["ArXiv Scout Report"]
    ...
    await log_event(f"ArXiv 보고서 생성 완료: {len(report_data)}개 쿼리")
